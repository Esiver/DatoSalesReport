...
def getSalesData(file):
    ...
    try:
        sales_data["date"] = pd.to_datetime(
            sales_data["date"], errors="coerce"
        )  # make a 'date'-column in datetime format
    except:
...
# format my original data into a dataframe friendly csv
def formatMain(mainFile):  # maybe 2 arguments instead? input and output name???
    csv_df = mainFile.to_csv()
    print(csv_df)
    print('DATA FRAME, main data')
    mainFile.to_csv(csv_df)  # make a csv file called r'all_data.csv'
